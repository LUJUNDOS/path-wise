 ...
 * 导出攻略
 *
 * 从 tripStore 读取攻略并生成格式化内容。
 * MVP 阶段：生成 HTML 文本，前端可直接下载或浏览器打印为 PDF。
 * 后续接入 CDN 后可返回 downloadUrl 指向预生成文件。
 */
export async function exportTrip(
  _tripId: string,
  _options: ExportOptions,
): Promise<ExportResponse> {
  const trip = tripStore.get(_tripId);
  if (!trip) {
...
function buildExportHtml(trip: TripResponse): string {
