 ...
 * 高德 POI 搜索（mock）
 */
export async function searchAmapPOI(_city: string, _keywords: string): Promise<POI[]> {
  return [];
}
 ...
 * 高德 POI 详情（mock）
 */
export async function getAmapPOIDetail(_poiId: string): Promise<POI | null> {
  return null;
}
 ...
 * 高德地理编码（mock）
 */
export async function geocodeAmap(_address: string): Promise<{ lat: number; lng: number } | null> {
  return null;
}
 ...
 * 高德天气查询（mock）
 */
export async function getAmapWeather(_cityCode: string): Promise<Record<string, unknown>> {
  return {
    forecast: '晴',
